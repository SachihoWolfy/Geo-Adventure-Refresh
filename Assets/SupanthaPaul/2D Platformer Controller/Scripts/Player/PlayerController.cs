...
namespace SupanthaPaul
{
	public class PlayerController : MonoBehaviour
	{
		...
		[SerializeField] private GameObject dashEffectL;
		[SerializeField] private float attackCooldown = 0.01f;
		[SerializeField] private GameObject bulletSpawner; //Bullet Stuff
		...
		void Start()
		{
			...
			PoolManager.instance.CreatePool(dashEffectL, 2);
		...
		private void Update()
		{
			...
			if (!isDashing && !m_hasDashedInAir && m_dashCooldown <= 0f)
			{
				...
				if (InputSystem.Dash())
				{
                    ...
                    {
						PoolManager.instance.ReuseObject(dashEffectL, transform.position, Quaternion.identity);
                    }
            ...
            {
				isAttacking = true;
				var exeBullet = Instantiate(bullet, bulletSpawner.transform.position, bulletSpawner.transform.rotation) as GameObject;
                if (!m_facingRight)
                {
					exeBullet.transform.localScale = new Vector3(-1f, 1f, 1f); // change its local scale in x y z format
				}
            } else if (!InputSystem.Attack())
            {
				isAttacking = false;
            }
		}
